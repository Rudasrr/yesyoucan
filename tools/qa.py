...
WIDTHS=[390,768,1440]
CHECK_JS='''(() => {
  const out={overflow:0, offscreen:[], contrast:[], tiny:[]};
  ...
  out.offscreen=out.offscreen.slice(0,5); out.contrast=out.contrast.slice(0,5); out.tiny=out.tiny.slice(0,5);
  return out; })()'''
rows=[]
with sync_playwright() as p:
    ...
    for role,views in [('client',VIEWS_C),('coach',VIEWS_K)]:
        for w in WIDTHS:
            ...
            for v in views:
                ...
                rows.append((role,w,v,res['overflow'],len(res['offscreen']),res['offscreen'][:2],len(res['contrast']),res['contrast'][:2],len(res['tiny']),nbtn,len(errs)))
            ctx.close()
    ...
    b.close()
print(f"{'role':6} {'w':5} {'view':10} {'ovfl':5} {'off':4} {'contr':5} {'tiny':4} {'btns':4} err")
for r in rows: print(f"{r[0]:6} {r[1]:<5} {r[2]:10} {r[3]:<5} {r[4]:<4} {r[6]:<5} {r[8]:<4} {r[9]:<4} {r[10]} {r[5] if r[4] else ''} {r[7] if r[6] else ''}")
