...
VIEWS.zprava = function () {
  ...
  <div class="grid g2"><div class="card"><h2>Co Robertovi říct</h2><ul class="qlist" style="margin-top:8px">${say.map(x => `<li>${esc(x)}</li>`).join('')}</ul><div class="row" style="margin-top:10px"><button class="btn sm" onclick="navigator.clipboard&&navigator.clipboard.writeText(${JSON.stringify(text).replace(/"/g, '&quot;')}).then(()=>UI.toast('Zpráva zkopírována do schránky'))">📋 Zkopírovat zprávu</button><button class="btn sec sm" onclick="saveCoachNote(${JSON.stringify(say[0]).replace(/"/g, '&quot;')});UI.toast('Poslána první věta jako vzkaz');render()">Poslat první větu jako vzkaz</button></div></div>
