...
VIEWS.dnes = function () {
  ...
  <div class="row between small" style="margin-top:6px"><span>■ snědeno ${fmt0(prog.eaten)} · ▢ ještě naplánováno ${fmt0(prog.planned)}${d.base.drinkKcal ? ` · pití ${fmt0(d.base.drinkKcal)}` : ''}</span><span>limit ${fmt0(d.base.maxIntake)}</span></div></div>
  <div class="stats3 four">
  ...
  if (c.active) {
    body = `<div class="tbl"><table class="items"><tr><th>Surovina</th><th class="n">g</th><th class="n m-kcal">kcal</th><th class="n m-prot">bílk.</th><th></th></tr>` +
      c.items.map(it => it.extra ? `<tr><td><button class="pickbtn sm edit" onclick="openFoodPicker(n=>A.extraFood('${cs.key}',${String(it.idx).slice(1)},n),${JSON.stringify(it.food).replace(/"/g, '&quot;')})">${esc(it.food)}</button><div class="tiny muted">přidáno</div></td>
        <td class="n"><span class="gstep"><button class="gb" onclick="A.gnudge(this,-10)">−</button><input class="g edit" type="number" min="0" step="5" value="${it.g}" onchange="A.extraG('${cs.key}',${String(it.idx).slice(1)},this.value)"><button class="gb" onclick="A.gnudge(this,10)">+</button></span></td><td class="n" data-l="kcal">${fmt0(it.kcal)}</td><td class="n" data-l="bílk.">${fmt1(it.p)}</td><td class="n"><button class="xbtn write" title="odebrat" onclick="A.extraDel('${cs.key}',${String(it.idx).slice(1)})">×</button></td></tr>`
      : `<tr><td>${modeBadge(it.food)}<button class="pickbtn sm ${it.swapped ? 'edit' : ''}" style="width:calc(100% - 26px)" onclick="openFoodPicker(n=>A.swap('${cs.key}',${it.idx},n),${JSON.stringify(it.food).replace(/"/g, '&quot;')})">${esc(it.food)}</button>${it.swapped ? `<div class="tiny muted">recept: ${esc(it.origFood)}</div>` : ''}</td>
        <td class="n">${measureText(it.food, it.g) ? `<div class="meas">${measureText(it.food, it.g)}</div>` : ''}<span class="gstep"><button class="gb" onclick="A.gnudge(this,-10)">−</button><input class="g ${it.manual ? 'edit' : ''}" type="number" min="0" step="5" value="${it.g}" onchange="A.gram('${cs.key}',${it.idx},this.value)"><button class="gb" onclick="A.gnudge(this,10)">+</button><span class="gu">g</span></span>${it.scale && !it.manual && it.g !== it.origG ? `<div class="tiny muted">recept ${it.origG} g · přizpůsobeno tvému limitu</div>` : ''}</td>
      ...
      `<tr class="sum"><td class="b">celkem</td><td></td><td class="n b" data-l="kcal">${fmt0(c.kcal)}</td><td class="n b" data-l="bílk.">${fmt1(c.p)}</td><td></td></tr></table></div>` +
      ...
      ${(m.extra || []).length ? `<div class="tbl"><table class="items" style="margin-top:8px"><tr><th>Surovina</th><th class="n">g</th><th class="n">kcal</th><th></th></tr>${(m.extra || []).map((ex, j) => { const f = foods.find(x => x.name === ex.food); return `<tr><td>${modeBadge(ex.food)}<button class="pickbtn sm edit" style="width:calc(100% - 26px)" onclick="openFoodPicker(n=>A.extraFood('${cs.key}',${j},n),${JSON.stringify(ex.food).replace(/"/g, '&quot;')})">${esc(ex.food)}</button></td><td class="n">${measureText(ex.food, ex.g) ? `<div class="meas">${measureText(ex.food, ex.g)}</div>` : ''}<span class="gstep"><button class="gb" onclick="A.gnudge(this,-10)">−</button><input class="g edit" type="number" min="0" step="5" value="${ex.g}" onchange="A.extraG('${cs.key}',${j},this.value)"><button class="gb" onclick="A.gnudge(this,10)">+</button><span class="gu">g</span></span></td><td class="n">${f ? fmt0(f.kcal * ex.g / 100) : ''}</td><td class="n"><button class="xbtn write" onclick="A.extraDel('${cs.key}',${j})">×</button></td></tr>`; }).join('')}<tr><td class="b">celkem</td><td></td><td class="n b">${fmt0((m.extra || []).reduce((a, ex) => { const f = foods.find(x => x.name === ex.food); return a + (f ? f.kcal * ex.g / 100 : 0); }, 0))}</td><td></td></tr></table></div><p class="hint">Do součtu dne se počítá cíl jídla; tvůj zápis je pro tebe a trenéra.</p>` : ''}`;
  ...
  return `<div class="card" id="cheat"><div class="row between"><h2>🍻 Cheat na dnešek${help('Plán, ne zápis. Když víš, že večer bude pivo, řízek nebo dort, zapiš to sem ráno. Appka to počítá do dnešního plánu a hlavně ti zvedne cíl chůze tak, aby tě to nestálo tempo. Nic se tu potom nepotvrzuje – je to plán na večer, ne záznam snědeného.')}</h2>${b.cheatKcal > 0 ? `<span class="pill">${fmt0(b.cheatKcal)} kcal</span>` : ''}</div>
    ...
    ${items.length ? `<div class="tbl" style="margin-top:8px"><table class="items"><tr><th>Co</th><th class="n">kolik</th><th class="n m-kcal">kcal</th><th></th></tr>
      ${items.map((it, i) => `<tr><td>${esc(it.food)}</td>
        <td class="n">${it.kcal != null ? `<span class="gstep"><button class="gb" onclick="A.gnudge(this,-50)">−</button><input class="g" type="number" min="0" step="50" value="${it.kcal}" onchange="A.cheatK(${i},this.value)"><button class="gb" onclick="A.gnudge(this,50)">+</button><span class="gu">kcal</span></span>` : `<span class="gstep"><button class="gb" onclick="A.gnudge(this,-10)">−</button><input class="g" type="number" min="0" step="10" value="${it.g}" onchange="A.cheatG(${i},this.value)"><button class="gb" onclick="A.gnudge(this,10)">+</button><span class="gu">g</span></span>`}</td>
