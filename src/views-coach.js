...
VIEWS.klient = function () {
  ...
  if (!App.dashDetail) return `<div class="row between" style="margin-bottom:8px"><h1>📊 Dashboard – ${esc((() => { const c = Store.clients.find(x => x.id === uid) || {}; return c.display_name || c.name || c.email || 'klient' })())}</h1><div class="chips">${[7, 14, 28].map(n => `<span class="chip ${N === n ? 'on' : ''}" onclick="App.dashRange=${n};render()">${n} dní</span>`).join('')}</div></div>${seedNote}${quiet}
  ...
  return `<div class="row between" style="margin-bottom:8px"><h1>📊 Dashboard – ${esc((() => { const c = Store.clients.find(x => x.id === uid) || {}; return c.display_name || c.name || c.email || 'klient' })())}</h1><div class="chips">${[7, 14, 28].map(n => `<span class="chip ${N === n ? 'on' : ''}" onclick="App.dashRange=${n};render()">${n} dní</span>`).join('')}</div></div>${seedNote}${quiet}
  ...
  <div class="grid g2">
    <div class="card"><h2>Váha proti plánu</h2>${lineChart({ series: [{ name: 'plán', color: '#9aa7ab', dash: true, pts: Array.from({ length: Math.max(84, (ov.last ? ov.last.idx : 0) + 14) + 1 }, (_, i) => [i, planWeightAt(s, i)]) }, { name: 'ranní váha', color: '#b9cfd2', pts: ov.rows.map(r => [r.idx, r.weight]), thin: true }, { name: 'průměr 7 dní', color: '#0ea5a4', pts: ov.rows.map(r => [r.idx, r.avg]), dots: true }], xLabel: 'dní od startu', yUnit: 'kg', h: 230, hLine: { y: s.goal_weight, label: 'cíl ' + s.goal_weight, color: '#16a34a' } })}</div>
    <div class="card"><h2>Příjem proti limitu (${N} dní)</h2>${intakePts.length ? lineChart({ series: [{ name: 'limit dne', color: '#9aa7ab', dash: true, pts: limitPts }, { name: 'příjem', color: '#ff7a45', pts: intakePts, dots: true }], xLabel: 'dny (0 = před ' + (N - 1) + ' dny)', yUnit: 'kcal', h: 230 }) : '<p class="muted small">Zatím žádný den s jídlem.</p>'}
...
A.editFood = (id, mode, after) => {
  ...
  const m = UI.modal(`<h2>${over ? 'Moje verze suroviny' : (id ? 'Upravit surovinu' : (own ? 'Moje surovina' : 'Nová surovina'))}</h2><p class="small muted" style="margin:4px 0 8px">Hodnoty na 100 g (nebo 100 ml) z obalu.${own ? ' Uvidíš ji jen ty a půjde použít v tvých receptech i při přidávání surovin.' : ''}${over ? ' Změna platí jen pro tebe – trenérova databáze zůstává. Název se nemění.' : ''}</p>
    ...
    <div class="row" style="margin-top:12px"><button class="btn" id="fsave">Uložit</button><button class="btn sec" onclick="UI.closeModal()">Zavřít</button>${id ? `<span class="sp"></span><button class="btn danger sm" id="fdel">${over ? 'Vrátit původní' : 'Smazat'}</button>` : ''}</div><div class="small muted" id="fuse" style="margin-top:8px"></div>`);
  ...
  const del = m.querySelector('#fdel'); if (del && over) del.onclick = () => { if (!src.ovId) { m.remove(); return; } Undo.run('Vrátit původní', () => { Store.remove('foods', src.ovId); }, `${f.name}: vrácena původní hodnota trenéra.`); m.remove(); render(); };
