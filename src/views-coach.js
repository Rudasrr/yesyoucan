...
VIEWS.klient = function () {
  ...
  if (!App.dashDetail) return `<div class="row between" style="margin-bottom:8px"><h1>📊 Dashboard – ${esc((Store.clients.find(c => c.id === uid) || {}).display_name || 'Robert')}</h1><div class="chips">${[7, 14, 28].map(n => `<span class="chip ${N === n ? 'on' : ''}" onclick="App.dashRange=${n};render()">${n} dní</span>`).join('')}</div></div>${seedNote}${quiet}
  ...
  return `<div class="row between" style="margin-bottom:8px"><h1>📊 Dashboard – ${esc((Store.clients.find(c => c.id === uid) || {}).display_name || 'Robert')}</h1><div class="chips">${[7, 14, 28].map(n => `<span class="chip ${N === n ? 'on' : ''}" onclick="App.dashRange=${n};render()">${n} dní</span>`).join('')}</div></div>${seedNote}${quiet}
